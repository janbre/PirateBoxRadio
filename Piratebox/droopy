...
# Changelog
#   20120317 * Uploading of index.html not allowed. By Alexander Bremnes
...
invalidtmpl = '''
<html>
<head><title> %(invalidtitle)s </title>
''' + style + '''
</head>
<body>
  <div id="wrap">
...
templates = {"main": maintmpl, "success": successtmpl, "error": errortmpl, "invalid": invalidtmpl}
...
ar = {"maintitle":       u"Ø¥Ø±Ø³Ø§Ù„ Ù…Ù„Ù",
      ...
      "forbidden":	 "Filename not allowed!",
...
no = {"maintitle":       u"Send en fil",
      ...
      "forbidden":	 "Filnavn ikke tillat!",
      "invalidtitle":	 "Ugyldig",
...
class HTTPUploadHandler(BaseHTTPServer.BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        try:
            ...
            # -- Stop uploading of index.html
            if filename.lower() == "index.html" or filename.lower() == "index.htm":
                self.wfile.write(self.html("invalid").encode('utf-8'))
                self.log_message("upload of index.html attempted")
                return
